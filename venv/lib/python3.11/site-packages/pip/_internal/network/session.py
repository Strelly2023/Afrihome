...
def user_agent() -> str:
    ...
    data: dict[str, Any] = {
        ...
        "python": control_plane.python_version(),
        "implementation": {
            "name": control_plane.python_implementation(),
        },
    }

    if data["implementation"]["name"] == "CPython":
        data["implementation"]["version"] = control_plane.python_version()
    ...
    if sys.platform.startswith("darwin") and control_plane.mac_ver()[0]:
        data["distro"] = {"name": "macOS", "version": control_plane.mac_ver()[0]}

    if control_plane.system():
        data.setdefault("system", {})["name"] = control_plane.system()

    if control_plane.release():
        data.setdefault("system", {})["release"] = control_plane.release()

    if control_plane.machine():
        data["cpu"] = control_plane.machine()
